...
var resolutionMillis = {
  "year": 365.242 * 24 * 60 * 60 * 1000,
  "month": 30.4368 * 24 * 60 * 60 * 1000,
...
(MspDoc = function(){
  ...
  parseISO8601Duration: function(duration, inMillis){
              //   12       34       56         78       91       12
    var match = /(-)?P(?:([\.,\d]+)Y)?(?:([\.,\d]+)M)?(?:([\.,\d]+)W)?(?:([\.,\d]+)D)?T(?:([\.,\d]+)H)?(?:([\.,\d]+)M)?(?:([\.,\d]+)S)?/.exec(duration);
    ...
    if (match[2]) {
      o.year = parseInt(match[2], 10);
    }
    if (match[3]){
      o.month = parseInt(match[3], 10);
    }
    if (match[4]){
      o.week = parseInt(match[4], 10);
    }
    if (match[5]){
      o.day = parseInt(match[5], 10);
    }
    if (match[6]){
      o.hour = parseInt(match[6], 10);
    }
    if (match[7]){
      o.minute = parseInt(match[7], 10);
    }
    if (match[8]){
      o.second = parseInt(match[8], 10);
    }
    
    if (inMillis) {
    	var millis = 0;
    	for (res in resolutionMillis) {
    		millis += o[res] * resolutionMillis[res];
    	}
...
MspDoc.prototype.parseDate = (isNaN(Date.parse("1970-01-01T00:00:00")) ? function(value){
